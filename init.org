...
** Visual
   ...
   Change the color-theme to =monokai-alt=.
   ...
   (defun cycle-themes ()
     ...
     (lexical-let ((themes '#1=(monokai-alt leuven zenburn . #1#)))
       (lambda ()
         (interactive)
...
** Helm
   ...
   #+END_SRC
** Projectile
   set ~alien~ indexing mode for Windows
...
** Yafolding
   Enabling ~yafolding-mode~ for ~json-mode~ and ~yafolding-mode~
   #+BEGIN_SRC emacs-lisp
...
** Flyspell
   ...
   #+BEGIN_SRC emacs-lisp
   ;; add check for win os
   (custom-set-variables '(ispell-program-name "C:\\Program Files (x86)\\Aspell\\bin\\aspell.exe"))
   #+END_SRC
